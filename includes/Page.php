...
class Page
{
	...
	private $page_full_name; # full page name (like Namespace:Title)
	private $namespace_id; # page namespace numeric ID as in page.page_namespace
	private $namespace_name; # page namespace name
	private $valid = false; # true if the page exists, false otherwise
	...
	private $average_rating; # average rating
	private $voters_count; # number of voters
	...
	private function reset()
	{
		...
		$this->voters_count = false;
		$this->namespace_name = '';
	}

	/** "load averages": sets $average_rating and $voters_count */
	private function loadAverages()
	{
		$working_data = wfGetDB( DB_REPLICA );

		$result = $working_data->select(
				'w4grb_avg',
				'avg,n',
		...
		if ($row = $working_data->fetchObject($result))
		{
			$this->average_rating = intval($row->avg);
			$this->voters_count = intval($row->n);
		}
		else
		{
			$this->average_rating = '--';
		...
		$working_data->freeResult($result);
		unset($working_data);
	}
	...
	* Getters
	**/
	public function getPageFullName()
	{
